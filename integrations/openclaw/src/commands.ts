...
async function handleRelayCommand(ctx: PluginCommandContext): Promise<{ text: string }> {
  ...
  const cronTo = buildCronTarget(target.peer, target.relayConversationId);
  ...
  const webhook = buildWebhookUrl(ctx.config as any, target.peer, target.relayConversationId) ?? buildWebhookPath(target.peer, target.relayConversationId);
  const curlSnippet = [
    `curl -X POST '${webhook}'`,
