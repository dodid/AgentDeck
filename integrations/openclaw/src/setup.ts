...
async function persistOpenClawConfig(cfg: OpenClawConfig): Promise<void> {
  const { writeOptions } = await readConfigFileSnapshotForWrite();
  await writeConfigFile(cfg, writeOptions);
}

async function writeSidecarAndPersistChannelPointer(
  cfg: OpenClawConfig,
  patch: Partial<SidecarSection>,
): Promise<OpenClawConfig> {
  writeSidecarSection(cfg, patch);
  const nextCfg = withChannelSection(cfg, configPointerPatch(cfg));
  await persistOpenClawConfig(nextCfg);
  return nextCfg;
}
...
export const r2RelaySetupWizard: ChannelSetupWizard = {
  ...
  status: {
    ...
    resolveStatusLines: ({ cfg, configured }) => {
      ...
      const lines = [
        configured
          ? "AgentDeck and other dedicated clients can use this channel once the gateway is running."
          : "This channel is intended for AgentDeck-style dedicated app onboarding.",
      ];
  ...
  introNote: {
    title: "R2 Relay by AgentDeck App",
    lines: [
  ...
  textInputs: [
    {
      ...
      applySet: async ({ cfg, value }) => await writeSidecarAndPersistChannelPointer(cfg, { endpoint: normalizeEndpoint(value) }),
    },
    {
      inputKey: "name",
      ...
      applySet: async ({ cfg, value }) => await writeSidecarAndPersistChannelPointer(cfg, { bucket: value.trim() }),
    },
    {
      inputKey: "userId",
      ...
      normalizeValue: ({ value }) => value.trim(),
      applySet: async ({ cfg, value }) => await writeSidecarAndPersistChannelPointer(cfg, { accessKeyId: value.trim() }),
    },
    {
      inputKey: "password",
      ...
      normalizeValue: ({ value }) => value.trim(),
      applySet: async ({ cfg, value }) => await writeSidecarAndPersistChannelPointer(cfg, { secretAccessKey: value.trim() }),
    },
    {
      inputKey: "deviceName",
      ...
      applySet: async ({ cfg, value }) => {
        const trimmed = value.trim();
        return await writeSidecarAndPersistChannelPointer(cfg, {
          serverId: trimmed ? normalizeServerId(trimmed) ?? trimmed : undefined,
        });
      },
    },
  ],
  completionNote: {
    ...
    lines: [
      ...
      "Then use AgentDeck or another dedicated OpenClaw app with the same R2 connection details.",
    ],
  },
};
