...
export function rememberConversationTarget(params: {
  ...
  store[key] = {
    ...
    peer: params.peer,
    relayConversationId: params.relayConversationId,
    updatedAt: Date.now(),
  };
...
export function buildCronTarget(peer: string, conversationId: string): string {
  return `peer=${peer},conversation=${conversationId}`;
}

export function buildWebhookPath(peer: string, conversationId: string): string {
  return `/r2-relay-channel/webhook/${encodeURIComponent(peer)}/${encodeURIComponent(conversationId)}`;
}

export function buildWebhookUrl(cfg: { gateway?: { publicBaseUrl?: string | null; externalUrl?: string | null; bindUrl?: string | null; port?: number | null; remote?: { url?: string | null } | null } }, peer: string, conversationId: string): string | null {
  const pathPart = buildWebhookPath(peer, conversationId);
