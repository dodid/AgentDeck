    ...
    private func content(for step: OnboardingViewModel.Step) -> some View {
        ...
        case .howItWorks:
            OnboardingPageView(
                ...
                disclaimer: "ClawChat is not affiliated with or endorsed by Cloudflare, Inc."
            ) {
    ...
    var body: some View {
        VStack(spacing: 26) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                OnboardingFeatureRow(icon: "shield.fill", color: AppTheme.blue, title: "No ports open", detail: "Your server stays safely behind its firewall. ClawChat never exposes it to the internet.")
    ...
    var body: some View {
        VStack(spacing: 24) {
            VStack(spacing: 10) {
                ...
                Text("ClawChat uses an R2 bucket as a private relay — a secure shared inbox between your phone and your agent platform server. No exposed ports.")
                    .font(.system(size: 15, weight: .regular))
                    .foregroundStyle(AppTheme.dim)
                    .multilineTextAlignment(.center)
                    .lineSpacing(3)
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                Divider().padding(.leading, 52)
                OnboardingRequirementRow(icon: "externaldrive.fill", title: "An R2 bucket", detail: "Cloudflare includes 10 GB of R2 free — more than enough for ClawChat.")
            }
            .background(AppTheme.panel)
            .overlay(RoundedRectangle(cornerRadius: 20).stroke(AppTheme.border, lineWidth: 1))
            .clipShape(RoundedRectangle(cornerRadius: 20))
...
private struct R2RelayDiagram: View {
    var body: some View {
        HStack(alignment: .top, spacing: 0) {
            diagramItem(icon: "iphone", label: "ClawChat", isAccent: false)
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                OnboardingNumberedStepRow(number: 2, text: "Create a new bucket — name it anything you like, e.g. **clawchat**.")
                Divider().padding(.leading, 34)
