...
The iOS component suite also covers onboarding and returning-install routing, connection replacement and local-data cleanup, discovery refresh recovery, composer send/failure/paywall behavior, backfill pagination, persistence, and relay protocol handling. StoreKit purchase UI, photo/speech system pickers, and provider model quality remain platform- or provider-owned behavior; they are smoke signals rather than deterministic release assertions.
